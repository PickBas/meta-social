...
class CommunityTest(MetaSetUp):
    ...
    def test_flow(self):
        url_create = reverse('community-create')
        ...
        url_com1 = reverse('community-page', kwargs={'community_url': com1.custom_url})
        ...
        resp = self.client.post(
            reverse('community-post-create', kwargs={'community_url': com1.custom_url}),
            {
                **form_data,
                **formset_data
            })
        ...
        response = self.client.get(reverse('post-page', kwargs={'post_id': p.id}))
...
class CommunityList(MetaSetUp):
    def setUp(self):
        super().setUp()

    def test_access_list(self):
        resp = self.client.get(reverse('community-user-list'))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, 'community/community_list.html')
        resp = self.client.get(reverse('profile-community-list'))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, 'community/own_community_list.html')

    def test_search(self):
        url_create = reverse('community-create')
        self.client.get(url_create)
        com_data = {
            'name': 'W40K',
            'info': '40 000',
            'country': 'RU',
        }
        self.client.post(url_create, com_data)
        com1 = Community.objects.get(owner=self.user, name=com_data['name'])
        resp = self.client.post(reverse('community-user-list'), {'query': 'W40'})
